...
| D-006 | 2026-08-03 | اعتماد Cloudflare Workers/Static Assets وCloudflare D1 على الخطة المجانية، مع Firebase Authentication Spark للمصادقة فقط. كل API خاص يعمل Fail closed، ويتحقق من Firebase ID Token وفق `alg=RS256` و`kid` المطابق لمفتاح Google العام والتوقيع و`aud` و`iss` و`exp` و`iat` و`auth_time` و`sub`، ثم قائمة سماح لشخصين داخل D1. ترتبط D1 بالـWorker داخليًا ولا يسمح بتجاوز Worker. تخزن كل القيم المالية كعدد صحيح من الهللات ضمن المجال الآمن، ويحظر floating point. يحظر Zero Trust وBlaze وأي Billing Account أو وسيلة دفع، ولا تنشأ خدمة سحابية دون موافقة مستقلة. | يحقق شرط صفر تكلفة إلزامية، ويوفر قاعدة علائقية قابلة للتصدير وهوية منفصلة، مع فصل مزود الهوية عن البيانات وتقليل مخاطر المصادقة المخصصة وقفل البيانات. تفاصيل التنفيذ والقيود في `docs/architecture/ADR-001-FREE-ARCHITECTURE.md` و`docs/architecture/FINANCIAL_INTEGER_RULE.md`. | معتمد |
