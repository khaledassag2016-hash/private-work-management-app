...
const state = {
  ...
  selectedWork: null,
  financial: { periodKey: new Date().toISOString().slice(0, 7), preview: null, snapshots: [], reopenRequests: [], transfers: [], subscriptions: [], expenses: [] },
  modal: null,
  busy: false,
};
...
async function loadFinancialWorkspace(periodKey = state.financial.periodKey) {
  ...
  const [preview, snapshots, reopenRequests, transfers, subscriptions, expenses] = await Promise.all([
    ...
    api('/api/transfers'), api('/api/subscriptions'), api('/api/expenses')
  ]);
  state.financial = { periodKey: period, preview, snapshots, reopenRequests, transfers, subscriptions, expenses };
  return state.financial;
}
async function refreshFinancialAfterMutation(message) {
  await loadFinancialWorkspace(); render(); toast(message, '');
}
