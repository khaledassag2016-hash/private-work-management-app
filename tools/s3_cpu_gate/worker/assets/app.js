...
function entityTypeLabel(value) { return ({ customer: 'عميل', work: 'عمل', work_event: 'حدث عمل', title_history: 'تاريخ عنوان', status_history: 'تاريخ حالة', financial_request: 'طلب مالي', payment: 'دفعة', payment_reversal_request: 'طلب تصحيح دفعة', settlement: 'تسوية', account_admin: 'إدارة حساب' }[value] || 'سجل'); }
...
function financialPage() {
  ...
  function s8AnalyticsMarkup() { const analytics = state.s8.analytics; if (!analytics) return empty('اضغط تحديث التحليل بعد تحديد أساس الفترة.'); const groups = analytics.groups || {}; return `<section class="grid grid-3" data-s8-analytics>${Object.entries(groups).map(([dimension, rows]) => `<article class="card"><h3>${escapeHtml(dimension)}</h3>${rows.length ? `<ul class="fact-list">${rows.map(row => `<li data-s8-analytics-row="${escapeHtml(dimension)}:${escapeHtml(row.bucket)}"><strong>${escapeHtml(row.bucket)}</strong><span>الأعمال: ${row.work_count} — نشطة: ${row.active_work_count} — مؤرشفة: ${row.archived_work_count}</span><span>القيمة: ${escapeHtml(moneyLabel(row.current_price_halalas))} — المدفوع: ${escapeHtml(moneyLabel(row.approved_paid_halalas))} — المتبقي: ${escapeHtml(moneyLabel(row.remaining_halalas))}</span></li>`).join('')}</ul>` : empty('لا توجد بيانات.')}</article>`).join('')}</section>`; }
...
async function submitS8AlertSettings(event) { event.preventDefault(); const values = formObject(event.currentTarget); const settings = ['NO_PRICE', 'NO_REPLY', 'NO_PAYMENT'].map(alert_type => ({ alert_type, threshold_days: values[`threshold_${alert_type}`] })).filter(setting => setting.threshold_days !== ''); if (!settings.length) { toast(errorMessage('S8_ALERT_SETTINGS_REQUIRED'), 'error'); return; } if (!(await confirmSensitive('حفظ إعدادات التنبيهات', settings.map(item => `${({ NO_PRICE: 'بلا سعر', NO_REPLY: 'بانتظار رد العميل', NO_PAYMENT: 'دون تحصيل' }[item.alert_type] || 'تنبيه')} — ${item.threshold_days} يوم`).join('، ')))) return; await submitFlow(async () => { for (const setting of settings) await api('/api/alerts/settings', { method: 'POST', body: setting }); await loadS8Workspace(); render(); toast('تم حفظ مدد التنبيهات.', ''); }); }
  ...
  async function submitS8Export(event) { event.preventDefault(); const values = formObject(event.currentTarget); state.s8.export_type = values.export_type; state.s8.export_work_id = values.work_id || ''; state.s8.export_customer_id = values.customer_id || ''; if (values.export_type === 'WORK' && !values.work_id) { toast('اختر عملًا للتصدير.', 'error'); return; } if (values.export_type === 'CUSTOMER' && !values.customer_id) { toast('اختر عميلًا للتصدير.', 'error'); return; } await submitFlow(async () => { const dto = await fetchS8CompleteExport(values.export_type, values); const exporter = await import(appConfig.s8ExportModuleUrl || '/assets/s8-export.mjs'); const bytes = exporter.generateS8Workbook(dto); const blob = new Blob([bytes], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = exporter.safeS8ExportFilename(values.export_type, values.work_id || values.customer_id || `${state.s8.filters.year || 'period'}-${state.s8.filters.month || 'all'}`); anchor.click(); URL.revokeObjectURL(url); toast('تم إنشاء XLSX من DTO السلطوي الحقيقي.', ''); }); }
  function dashboard() {
...
function auditPage() {
  ...
  return `<section class="card" data-audit-log><div class="toolbar"><div><h2>سجل التدقيق</h2><p>عرض قراءة فقط للسجل السلطوي، مرتب من الأحدث إلى الأقدم حسب التاريخ.</p></div><button class="button secondary" data-action="audit-refresh" type="button" ${state.audit.loading ? 'disabled' : ''}>تحديث</button></div><p class="notice info">الحذف النهائي غير متاح؛ يعرض السجل التغييرات الفعلية وتاريخ الأرشفة أو الإلغاء عند انطباقه.</p>${state.audit.loading ? loading() : rows.length ? `<div class="table-wrap"><table class="audit-table"><thead><tr><th>العملية</th><th>نوع السجل ومعرفه</th><th>من قام بالعملية</th><th>التاريخ والوقت</th><th>القيمة السابقة</th><th>القيمة الجديدة</th></tr></thead><tbody>${rows.map(row => `<tr data-audit-id="${escapeHtml(row.id)}"><td>${escapeHtml(auditActionLabel(row.action))}</td><td>${escapeHtml(entityTypeLabel(row.entity_type))}<br/><span class="hint">سجل محفوظ</span></td><td>${escapeHtml(roleLabel(row.actor_role))}<br/><span class="hint">حساب مصرح</span></td><td>${escapeHtml(dateTimeLabel(row.created_at))}</td><td><pre class="audit-json">${escapeHtml(auditValueLabel(row.before))}</pre></td><td><pre class="audit-json">${escapeHtml(auditValueLabel(row.after))}</pre></td></tr>`).join('')}</tbody></table></div>` : empty('لا توجد سجلات تدقيق متاحة.')}</section>`;
}
...
function workPage() {
  ...
  <section class="grid grid-2" data-execution-collection-separation>
    ...
    <article class="card" data-collection-status>
      ...
      <p class="hint">المتبقي المعروض قراءة تمهيدية من السجل المالي المعتمد؛ لا تنشئ الواجهة مصدرًا ماليًا مستقلًا.</p>
    </article>
  </section>
  ...
  <section class="grid grid-2">
    <article class="card">
      ...
      <div class="fact-list">
        ...
        <li><strong>النوع والتخصص</strong><span>${escapeHtml(workTypeLabel(work.work_type_key))} — ${escapeHtml(work.specialty_key || 'غير محدد')}</span></li>
...
async function handleApprovePriceRequest(reqId) {
  ...
  if (!(await confirmSensitive('اعتماد حركة السعر', `السعر الحالي ${moneyLabel(state.selectedWork.financials?.current_price_halalas)}؛ سيصبح الطلب جزءًا من السجل المعتمد`))) return;
  await submitFlow(async () => {
    try {
