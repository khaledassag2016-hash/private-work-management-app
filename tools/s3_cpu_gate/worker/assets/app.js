...
const state = {
  ...
  s8: { filters: { q: '', period_basis: 'CREATED_AT', month: '', year: '', status: '', customer_id: '', country: '', university: '', specialty_key: '', work_type_key: '', collection_status: '', include_archived: false }, search: { items: [], page: 1, page_size: 25, has_more: false }, analytics: null, alerts: null, export_type: 'WORK', export_work_id: '', export_customer_id: '', loading: false },
  modal: null,
  busy: false,
};
...
const labels = {
  ...
  S8_ALERT_CLOCK_ANCHOR_BLOCKED: 'لا توجد ساعة حاكمة معتمدة؛ بقيت تنبيهات الساعة Fail-Closed.',
};
...
function financialPage() {
  ...
  async function loadS8Workspace() { state.s8.loading = true; try { await Promise.all([loadS8Search(), loadS8Analytics(), api('/api/alerts').then(data => { state.s8.alerts = data; })]); } finally { state.s8.loading = false; } return state.s8; }
