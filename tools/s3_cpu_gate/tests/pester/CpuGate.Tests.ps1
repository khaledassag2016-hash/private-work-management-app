...
Describe 'B2 Workers Observability telemetry query' -Tag 'B2' {
 ...
 It 'uses the current schema and reads all telemetry pagination pages' {
  ...
  Mock Invoke-S3CloudflareRest {param($Method,$Uri,$Token,$Body)
   ...
   $Body.queryId | Should -Match '^s3cpu-run-b2-scenario-a-';$Body.timeframe.from | Should -BeOfType [long];$Body.timeframe.to | Should -BeGreaterThan $Body.timeframe.from;$Body.dry | Should -BeTrue;$Body.parameters.filterCombination | Should -Be 'and';$Body.parameters.filters.Count | Should -Be 2;$Body.parameters.filters[0].type | Should -Be 'string';$Body.PSObject.Properties.Name | Should -Not -Contain 'page';$Body.PSObject.Properties.Name | Should -Not -Contain 'cursor'
   $script:page++
   if($script:page -eq 1){[pscustomobject]@{success=$true;errors=@();result=[pscustomobject]@{events=[pscustomobject]@{count=2;events=@([ordered]@{'$metadata'=[ordered]@{id='event-a'};'$metadata.runId'='run-b2';'$metadata.requestId'='a';'$metadata.scenario'='scenario-a';'$workers.requestId'='cf-a';'$workers.cpuTimeMs'=2;'$workers.wallTimeMs'=8;'$workers.outcome'='ok'})}}}}
   else{$Body.offset | Should -Be 'event-a';[pscustomobject]@{success=$true;errors=@();result=[pscustomobject]@{events=[pscustomobject]@{count=2;events=@([ordered]@{'$metadata'=[ordered]@{id='event-b'};'$metadata.runId'='run-b2';'$metadata.requestId'='b';'$metadata.scenario'='scenario-a';'$workers.requestId'='cf-b';'$workers.cpuTimeMs'=3;'$workers.wallTimeMs'=9;'$workers.outcome'='ok'})}}}}
 ...
 It 'fails incomplete current pagination metadata' {Mock Invoke-S3CloudflareRest {[pscustomobject]@{success=$true;errors=@();result=[pscustomobject]@{events=[pscustomobject]@{events=@()}}}} -ModuleName Cloudflare;$q=Invoke-S3WorkersTelemetryQuery -AccountId account -Token token -RunId run-b2 -Scenario scenario-a;$q.status|Should -Be 'FAIL';$q.errors|Should -Contain 'PAGINATION_METADATA_INCOMPLETE'}
