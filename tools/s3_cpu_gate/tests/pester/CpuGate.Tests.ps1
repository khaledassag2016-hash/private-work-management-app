...
Describe 'B2 Workers Observability telemetry query' -Tag 'B2' {
 ...
 It 'waits through an early cache-state mismatch and still fails at the bounded timeout' {
  ...
  Mock Invoke-S3WorkersTelemetryQuery {$script:telemetryPoll++;if($script:telemetryPoll -lt 4){$partial=Get-B2RecordFixture 'a';$partial.cache_state='miss';Get-B2QueryResultFixture @($partial)}else{Get-B2QueryResultFixture @((Get-B2RecordFixture 'a'),(Get-B2RecordFixture 'b'))}} -ModuleName CpuGate
