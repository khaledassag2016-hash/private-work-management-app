...
Describe 'B2 Workers Observability telemetry query' -Tag 'B2' {
 ...
 It 'does not contain a wrangler tail fallback' {$source=Get-Content (Join-Path $SourceRoot 'src\modules\CpuGate.psm1') -Raw;$source|Should -Not -Match '(?i)wrangler.*tail|Start-S3Tail|ConvertFrom-S3TailEvent';$source|Should -Match '/workers/observability/telemetry/query'}
}
