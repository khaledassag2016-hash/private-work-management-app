...
Describe 'B2 Workers Observability telemetry query' -Tag 'B2' {
 ...
 It 'waits through an early cache-state mismatch and still fails at the bounded timeout' {
  $e=Get-B2ExpectedFixture @('a');Mock Invoke-S3WorkersTelemetryQuery {Get-B2QueryResultFixture @()} -ModuleName CpuGate;$fixed=[DateTime]'2026-08-05T08:00:00Z';$timeout=Wait-S3WorkersTelemetry -AccountId account -Token token -RunId run-b2 -Scenario scenario-a -ExpectedRequests $e -WorkerName s3-worker -FromUtc ([DateTime]'2026-08-05T07:59:00Z') -TimeoutSeconds 0 -RetryDelaySeconds 0 -Now {$fixed} -Sleep {param($Seconds);[void]$Seconds};$timeout.status|Should -Be 'FAIL';$timeout.reasons|Should -Contain 'TELEMETRY_TIMEOUT'
  $e=Get-B2ExpectedFixture @('a','b');$script:telemetryPoll=0
  Mock Invoke-S3WorkersTelemetryQuery {$script:telemetryPoll++;if($script:telemetryPoll -lt 4){$partial=Get-B2RecordFixture 'a' -Cpu $null -Wall $null -Outcome '';$partial.cache_state='miss';$partial.hasInvocation=$false;Get-B2QueryResultFixture @($partial)}else{Get-B2QueryResultFixture @((Get-B2RecordFixture 'a'),(Get-B2RecordFixture 'b'))}} -ModuleName CpuGate
  $fixed=[DateTime]'2026-08-05T08:00:00Z';$r=Wait-S3WorkersTelemetry -AccountId account -Token token -RunId run-b2 -Scenario scenario-a -ExpectedRequests $e -WorkerName s3-worker -FromUtc ([DateTime]'2026-08-05T07:59:00Z') -ExpectedCacheState hit -RetryDelaySeconds 0 -Now {$fixed} -Sleep {param($Seconds);[void]$Seconds};$r.status|Should -Be 'PASS';$script:telemetryPoll|Should -Be 4
 }
