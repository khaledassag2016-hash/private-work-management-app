...
test('S8 PR-B generates real secure RTL XLSX workbooks with exact sheets, Arabic headers, numeric money, and safe formula-like text', async () => {
  ...
  try {
    ...
    const workBook = assertWorkbook(generateS8Workbook(cases[0][0]), cases[0][1]); const summary = workBook.Sheets['ملخص العمل']; assert.equal(Math.round(summary.K2.v * 100), 10000); assert.equal(summary.K2.t, 'n'); assert.equal(summary.A1.v, 'العميل'); assert.equal(XLSX.utils.decode_range(summary['!ref']).e.r, 1); assert.equal(workBook.Props.Title, 'تقرير إدارة الأعمال الخاصة'); assert.ok(!Object.values(summary).some(cell => cell?.v && /UTC|معرف/.test(String(cell.v))));
    const followUpBook = assertWorkbook(generateS8Workbook(cases[2][0]), cases[2][1]); const safeEvent = followUpBook.Sheets['سجل المتابعة'].D2; assert.equal(safeEvent.f, undefined); assert.equal(safeEvent.v, '=SUM(1,1)');
    const settlementSheet = parseS8Workbook(generateS8Workbook(cases[1][0])).Sheets['التسوية']; assert.equal(settlementSheet.F1.v, 'حصة الطرف الأول من الأعمال بالريال'); assert.equal(settlementSheet.G1.v, 'حصة الطرف الثاني من الأعمال بالريال'); assert.ok(!Object.values(settlementSheet).some(cell => cell?.v && /snapshot|الشخص 1|الشخص 2|حالة عدم الحسم/.test(String(cell.v))));
    const edgeDto = structuredClone(cases[0][0]); edgeDto.work.current_price_halalas = 9007199254740990; edgeDto.work.approved_paid_halalas = 0; edgeDto.work.remaining_halalas = 9007199254740990; const edgeBook = parseS8Workbook(generateS8Workbook(edgeDto)); const edgePrice = edgeBook.Sheets['ملخص العمل'].K2; assert.equal(edgePrice.t, 's'); assert.equal(edgePrice.v, '90071992547409.90');
    assert.equal(safeS8ExportFilename('WORK', '../../unsafe/اسم'), 'تقرير-عمل-unsafe-اسم.xlsx'); assert.throws(() => safeS8ExportFilename('INVALID', 'x'), /S8_EXPORT_TYPE_INVALID/);
  } finally { database.close(); }
});
