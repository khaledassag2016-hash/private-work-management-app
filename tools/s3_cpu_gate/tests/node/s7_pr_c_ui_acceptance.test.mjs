...
test('S7 PR-C integrated UI plus Worker/DB acceptance covers confirmation, C/F/H/I/K/L/M/O without local financial truth', async () => {
  ...
  try {
    ...
    const confirmed = await worker.getWork(env, work.id); assert.equal(confirmed.confirmed_at, '2026-08-12T10:00:00.000Z'); assert.equal((await worker.getSettlementPreview(env, '2026-08', {})).work_count, 1); assert.match(ui.workPage(), /تاريخ التأكيد/);
    ...
    await ui.loadFinancialWorkspace('2026-08'); assert.match(ui.s7WorkFinancialMarkup(state.selectedWork), /name="received_by"/); assert.match(ui.settlementPreviewMarkup(state.financial.preview), /استلام الشخص الأول/);
