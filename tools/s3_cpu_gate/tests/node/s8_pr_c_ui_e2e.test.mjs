...
test('S8 PR-C authenticates every UI request through Worker, configures D-017 FR-029, proves archive scope, and produces real XLSX from DTOs', { skip: !(() => { try { execFileSync('openssl', ['version'], { stdio: 'ignore' }); return true; } catch { return false; } })() }, async () => {
  ...
  try {
    ...
    let html = ui.s8SearchPage(); assert.match(html, /data-s8-search-panel/); assert.match(html, /Current Active Work/); assert.doesNotMatch(html, /Archived Current Title/); assert.match(html, /تنبيهات FR-029/); assert.doesNotMatch(html, /CLOCK_ANCHOR_UNRESOLVED|FR-029 PASS/); assert.equal(state.s8.alerts.alerts.every(alert => alert.state === 'NOT_CONFIGURED'), true); assert.match(html, /غير مضبوط/);
