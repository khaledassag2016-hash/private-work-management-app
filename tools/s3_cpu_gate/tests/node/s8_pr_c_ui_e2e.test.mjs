...
test('S8 PR-C authenticates every UI request through Worker, configures D-017 FR-029, proves archive scope, and produces real XLSX from DTOs', { skip: !(() => { try { execFileSync('openssl', ['version'], { stdio: 'ignore' }); return true; } catch { return false; } })() }, async () => {
  ...
  try {
    ...
    let html = ui.s8SearchPage(); assert.match(html, /بحث وتحليلات وتصدير S8/); assert.match(html, /Current Active Work/); assert.doesNotMatch(html, /Archived Current Title/); assert.match(html, /تنبيهات FR-029/); assert.doesNotMatch(html, /CLOCK_ANCHOR_UNRESOLVED|FR-029 PASS/); assert.equal(state.s8.alerts.alerts.every(alert => alert.state === 'NOT_CONFIGURED'), true); assert.match(html, /غير مضبوط/);
    await ui.submitS8AlertSettings(form({ threshold_NO_PRICE: '1', threshold_NO_REPLY: '1', threshold_NO_PAYMENT: '1' })); assert.equal(state.s8.alerts.alerts.every(alert => alert.state === 'CONFIGURED'), true); const alertByType = new Map(state.s8.alerts.alerts.map(alert => [alert.alert_type, alert])); assert.ok(alertByType.get('NO_PRICE').items.some(item => item.work_id === noReply.id)); assert.ok(alertByType.get('NO_REPLY').items.some(item => item.work_id === noReply.id)); assert.ok(alertByType.get('NO_PAYMENT').items.some(item => item.work_id === active.id)); html = ui.s8SearchPage(); assert.match(html, /CONFIGURED/); assert.match(html, /Waiting Client Reply/); assert.doesNotMatch(html, /Fail-Closed: لا توجد ساعة حاكمة/);
